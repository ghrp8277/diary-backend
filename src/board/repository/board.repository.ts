...
export class BoardRepository extends Repository<Board> {
  // 게시글 생성
  async createBoard(user_id: number, boardModule: BoardInterface): Promise<Board> {
    const { title, content, datetime, isMember } = boardModule;
  ...
  async findAllBoardByUsername(
      ...
      .where('user_member.username = :username OR user_oauth.username = :username', { username })
  ...
  // 게시글 수정
  async updateBoardById(id: number, boardModule: { title: string, content: string, datetime: Date }): Promise<void> {
